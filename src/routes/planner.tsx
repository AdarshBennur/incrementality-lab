...
import { requiredSampleSize } from "@/lib/stats";
...
export const Route = createFileRoute("/planner")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Design a rigorous marketing experiment before spending: sample size, duration, power, and readiness in one interactive tool." },
      { property: "og:title", content: "Experiment Planner — Causal Lab" },
      { property: "og:description", content: "Design a rigorous marketing experiment before spending: sample size, duration, power, and readiness in one interactive tool." },
    ...
    <AppShell>
      <div className="mb-10">
        ...
        <p className="mt-5 max-w-2xl text-base text-muted-foreground leading-relaxed">
          Tune the inputs on the left and watch sample size, duration, and expected business impact update instantly.
          A well-planned experiment is the cheapest one you'll ever run.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[440px_1fr] gap-8">
        ...
        <Card className="lg:p-8 space-y-6 lg:sticky lg:top-24 self-start">
          ...
          <div className="grid grid-cols-2 gap-4">
            <NumberField label="Daily traffic" value={dailyTraffic} onChange={setDailyTraffic} suffix="users/day" />
            <NumberField label="Avg. conversion value" value={aov} onChange={setAov} prefix="$" />
          </div>

          <NumberField label="Campaign budget (optional)" value={budget} onChange={setBudget} prefix="$" />
        </Card>

        {/* Outputs */}
        <div className="space-y-6">
          <SectionHeading eyebrow="Outputs" title="What this experiment will require"
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Card className="p-5">
              ...
              <div className="font-serif text-3xl num text-primary mt-2">{fmtNum(days)}<span className="text-lg text-muted-foreground ml-1">days</span></div>
              <div className="text-xs text-muted-foreground mt-1 num">at {fmtNum(dailyTraffic)}/day</div>
            </Card>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card className="lg:col-span-2 lg:p-8">
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <Stat label="Potential incremental revenue" value={fmtCurrency(expectedIncRevenue)} />
                <Stat label="Expected treatment conversions" value={fmtNum(Math.round(result.nTreat * baseline * (1 + mde)))} />
                <Stat label="Expected control conversions" value={fmtNum(Math.round(result.nCtrl * baseline))} />
              </div>
              <div className="mt-6 pt-6 hairline text-sm text-muted-foreground leading-relaxed">
                A {(mde*100).toFixed(1)}% lift on a {(baseline*100).toFixed(2)}% baseline is a small movement in absolute terms —
                which is exactly why the calculator asks for so many users. The smaller the effect you want to catch,
                the more sample you need to catch it reliably.
              </div>
            </Card>
            ...
            <Card className={`lg:p-8 border-${readiness.color}/30`}>
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className={`font-serif text-2xl text-${readiness.color === "success" ? "success" : readiness.color === "warning" ? "warning" : readiness.color === "destructive" ? "destructive" : "ink"}`}>
                  {readiness.label}
                </div>
              </div>
          ...
          <Card className="lg:p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm">
              <Sensitivity label="Detect half the lift" note={`MDE = ${(mde/2*100).toFixed(1)}%`}
                value={fmtNum(requiredSampleSize({ baseline, mde: mde/2, confidence, power, splitTreat }).total)} />
              <Sensitivity label="Double the daily traffic" note={`${fmtNum(dailyTraffic*2)} users/day`}
                value={`${Math.max(1, Math.ceil(result.total/(dailyTraffic*2)))} days`} />
              <Sensitivity label="Require 95% power" note="From current setting"
                value={fmtNum(requiredSampleSize({ baseline, mde, confidence, power: 0.95, splitTreat }).total)} />
            </div>
          </Card>
          ...
          <Card className="lg:p-8">
            <ul className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
              {[
                { title: "Primary KPI defined", detail: "One metric wins or loses the experiment." },
                { title: "Treatment / control structure", detail: `Randomized ${Math.round(splitTreat*100)}/${Math.round((1-splitTreat)*100)} assignment at the user level.` },
                { title: "Minimum detectable effect", detail: `Committed to detecting a ${(mde*100).toFixed(1)}% relative lift.` },
                { title: "Sample size", detail: `${fmtNum(result.total)} users allocated across both arms.` },
                { title: "Experiment duration", detail: `${days} days at ${fmtNum(dailyTraffic)} users/day.` },
                { title: "Guardrail metrics", detail: "Bounce rate, cost/conversion, CX complaints tracked." },
                { title: "Success criteria written", detail: "Decision rules agreed before results are looked at." },
                { title: "Instrumentation validated", detail: "Assignment, exposure, and conversion events QA'd end-to-end." },
              ].map((c) => (
...
}: { label: string; hint?: string; value: number; min: number; max: number; step: number; display: string; onChange: (v: number) => void }) {
  return (
    <div>
...
function Sensitivity({ label, note, value }: { label: string; note: string; value: string }) {
  return (
    <div className="rounded-lg border border-border bg-cream/50 p-4">
      <div className="flex items-center gap-1.5 editorial-eyebrow"><Info className="h-3 w-3" /> {label}</div>
...
function getReadiness({ days, total, budget, expectedIncRevenue, mde }: {
  days: number; total: number; budget: number; expectedIncRevenue: number; mde: number;
}) {
  if (days > 60) return { label: "Too Short on Traffic", color: "warning", icon: <AlertTriangle className="h-5 w-5" />,
  ...
  if (total > 1_500_000) return { label: "Over-sized", color: "warning", icon: <AlertTriangle className="h-5 w-5" />,
    detail: `${total.toLocaleString()} users is a heavy commitment. If you can accept a larger MDE, sample size drops sharply.` };
  if (mde < 0.02) return { label: "Underpowered risk", color: "warning", icon: <AlertTriangle className="h-5 w-5" />,
    detail: `A ${(mde*100).toFixed(1)}% MDE is very tight. Tiny effects need enormous samples — make sure you truly need this precision.` };
  if (budget && expectedIncRevenue < budget * 0.4) return { label: "Weak expected ROI", color: "destructive", icon: <XCircle className="h-5 w-5" />,
    detail: "Even if the lift holds, expected incremental revenue is well below your budget. Reconsider MDE assumptions or spend." };
  return { label: "Ready", color: "success", icon: <CheckCircle2 className="h-5 w-5" />,
