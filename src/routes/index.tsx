...
import {
  TreatmentVsControlBar, ConversionTrend, CumulativeLift, OutcomesBar,
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Executive summary of a randomized marketing experiment: lift, incrementality, and statistical verdict." },
      { property: "og:title", content: "Experiment Overview — Causal Lab" },
      { property: "og:description", content: "Executive summary of a randomized marketing experiment: lift, incrementality, and statistical verdict." },
    ...
    <AppShell>
      ...
      <Card className="mb-10 flex flex-col lg:flex-row lg:items-center gap-6 lg:gap-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 lg:border-l lg:border-border lg:pl-10">
          ...
          <Stat label="P-value" tipTerm="P-value" value={a.test.pValue < 0.0001 ? "< 0.0001" : a.test.pValue.toFixed(4)} />
          <Stat label="95% CI (abs)" tipTerm="Confidence Interval"
      ...
      {/* KPI grid */}
      <SectionHeading eyebrow="At a Glance" title="Executive KPIs"
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-16">
        <KpiCard label="Treatment CVR" value={fmtPctRaw(a.test.pTreat * 100)} sub={`${fmtNum(exp.convTreat)} of ${fmtNum(exp.nTreat)}`} />
        <KpiCard label="Control CVR" value={fmtPctRaw(a.test.pCtrl * 100)} sub={`${fmtNum(exp.convCtrl)} of ${fmtNum(exp.nCtrl)}`} />
        <KpiCard label="Absolute Lift" tipTerm="Absolute Lift" tone="primary"
          value={fmtSigned(a.test.absLift * 100, 2, "pp")} sub="Percentage points" />
        <KpiCard label="Relative Lift" tipTerm="Relative Lift" tone={a.test.relLift >= 0 ? "positive" : "negative"}
          value={fmtSigned(a.test.relLift * 100, 2, "%")} sub="vs control baseline" />
        <KpiCard label="Incremental Conversions" value={fmtNum(Math.round(a.incrementalConv))}
          sub={`of ${fmtNum(exp.convTreat)} observed`} />
        <KpiCard label="Incremental Revenue" tone="primary"
          value={fmtCurrency(a.incrementalRevenue)} sub={`${fmtPct(a.incrementalityPct, 1)} of attributed`} />
        <KpiCard label="Statistical Significance" tipTerm="Statistical Significance"
          value={a.test.significant ? "Yes" : "No"} sub={`p = ${a.test.pValue < 0.0001 ? "<0.0001" : a.test.pValue.toFixed(4)}`} />
        <KpiCard label="Incremental ROAS" tipTerm="Incremental ROAS" tone={a.incrementalRoas >= 1 ? "positive" : "negative"}
          value={`${a.incrementalRoas.toFixed(2)}×`} sub={`vs ${a.traditionalRoas.toFixed(2)}× reported`} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2 lg:p-10 space-y-5 leading-relaxed">
          ...
          <p className="font-serif text-2xl leading-snug text-ink">
            The {exp.channel} test <em className="not-italic text-primary">{a.test.significant ? "produced a real, measurable lift" : "did not produce a lift we can trust"}</em>{" "}
            on {exp.primaryKpi.toLowerCase()} — moving it from{" "}
            <span className="num">{fmtPctRaw(a.test.pCtrl * 100)}</span> to{" "}
            <span className="num">{fmtPctRaw(a.test.pTreat * 100)}</span>{" "}
            over {a.durationDays} days.
          </p>
          <p className="text-sm text-muted-foreground">
            The observed <span className="text-ink">{fmtSigned(a.test.relLift*100, 2, "%")}</span> relative lift is
            {a.test.significant
          ...
          <p className="text-sm text-muted-foreground">
            After removing the {fmtNum(Math.round(a.baselineConv))} conversions the control group tells us would have happened anyway,
            the campaign is responsible for roughly <span className="text-ink num">{fmtNum(Math.round(a.incrementalConv))}</span> incremental conversions,
            or <span className="text-ink num">{fmtCurrency(a.incrementalRevenue)}</span> in genuine incremental revenue —{" "}
            <span className="text-ink num">{fmtPct(a.incrementalityPct, 0)}</span> of the {fmtCurrency(a.attributedRevenue)} last-click attribution would suggest.
          </p>
          <p className="text-sm text-muted-foreground">
            <span className="text-ink">Recommendation.</span>{" "}
            {a.decision === "SCALE" && "The incremental ROAS clears the payback bar with room to spare. Scale spend, keep a holdout, and monitor for saturation."}
            {a.decision === "MAINTAIN" && "Incremental economics are positive but not exceptional. Hold current investment and iterate on creative to expand the winning pattern."}
            {a.decision === "ITERATE" && "The effect is too small (or too noisy) to justify scaling. Iterate on targeting or creative and re-test with a larger sample."}
            {a.decision === "STOP" && "Incremental returns don't cover the spend. Pause the campaign in its current form and redesign before re-launching."}
          </p>
        </Card>
        ...
        <div className="space-y-4">
          ...
          <Card className="space-y-4">
            ...
            <div className="font-serif text-xl text-ink">{a.decision.toLowerCase().replace(/^\w/, c => c.toUpperCase())} the campaign</div>
            <div className="text-sm text-muted-foreground">
