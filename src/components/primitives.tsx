  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 sm:gap-6 mb-6">
      <div>
        ...
        <h2 className="font-serif text-[1.75rem] sm:text-3xl md:text-[2rem] leading-[1.1] tracking-tight text-ink">
          {title}
        </h2>
...
export function KpiCard({
  ...
  return (
    <div className="card-elev p-5 sm:p-6 flex flex-col gap-3">
      <div className="flex items-center gap-1.5">
        <div className="editorial-eyebrow">{label}</div>
        {tipTerm && <InfoTip term={tipTerm} />}
      </div>
      <div className={cn("font-serif text-3xl sm:text-4xl leading-none num", toneClass)}>
        {value}
      </div>
...
export function VerdictBadge({ significant }: { significant: boolean }) {
  return (
    <div
      className={cn(
        "inline-flex items-center gap-2 rounded-full border px-2.5 sm:px-3 py-1 text-[10px] sm:text-[11px] font-medium uppercase tracking-[0.14em]",
        significant
...
export function DecisionBadge({
  ...
  return (
    <div
      className={cn(
        "inline-flex items-center gap-2 rounded-md px-4 py-2 font-medium tracking-[0.14em] text-sm",
        map.bg,
        map.text,
      )}
    >
